...
public class JobQueue {
    ...
    public JobQueue(ThumbnailRepository thumbnailRepository) {
            ...
            case THUMBNAIL_FUNCTION -> {
                if (job instanceof ThumbnailJob) {
                    try {
                        ...
                        ThumbnailMetadata thumbnailMetadata = thumbnailRepository.createAndSaveThumbnailDefaultSettings(((ThumbnailJob) job).getOriginalFolderPath(), ((ThumbnailJob) job).getOriginalFilename(), ((ThumbnailJob) job).getFileId()).get();
